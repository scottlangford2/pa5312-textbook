---
layout: page
title: "Chapter 7: Regression for Evaluation"
nav_label: "Ch 7"
...
This same machinery handles a **binary** outcome. When $$Y_i$$ is coded 0/1 — did the child graduate (Perry, Case E), was the client employed — regressing it on a treatment dummy is a *linear probability model*, and $$\tau$$ is read directly as a change in probability: it recovers exactly the **risk difference** of Chapter 5. Regressing Perry's 0/1 graduation outcome on its treatment indicator, with no controls, returns $$\tau \approx 0.21$$ — the same 21-percentage-point gap the two-proportion test found. Regression does not require a dollar-valued outcome; it simply lets you *add covariates* to a comparison you could already run.
